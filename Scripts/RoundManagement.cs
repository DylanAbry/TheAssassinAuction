...
{
    public IntroAndRules introScript;
    public WeaponManager weaponScript;
    public SideButtonManagement buttonScript;
    public SpeedGameHandler speedScript;
    public MemoryGameHandler memoryScript;
    public AccuracyGameHandler accuracyScript;
    public ResultsProcessor resultsScript;
    ...
    void Start()
    {
            ResultsProcessor.activeWeaponNames.Clear();
            numPlayerWeapons = 0;
            numFleeceWeapons = 0;
            numTradedItems = 0;
            tradeInsLeft = 3;
            ...
            pauseIcon.SetActive(false);
             ...
             if (ResultsProcessor.activeWeaponNames.Count == 0 || resultsScript.playerCollectionResults.Count == 0)
             {
    ...
    void Update()
    {
        ...
        if (increaseBidButton.enabled == true|| introScript.introPanels[8].activeInHierarchy)
        {
    ...
    public void IncreaseBid()
    {
        ...
        {
            if (currentItemPrice < 140 || roundTracker == 12)
            {
            ...
            {
                if (fleeceRaiseBidProbability < 65)
                {
                    StartCoroutine(FleeceFollowUpBids());
                }
            }
            ...
            {
                if (fleeceRaiseBidProbability < 35)
                {
                    StartCoroutine(FleeceFollowUpBids());
                }
            }
            ...
            {
                if (fleeceRaiseBidProbability < 15)
                {
                    StartCoroutine(FleeceFollowUpBids());
                }
            }
    ...
    public void IntermissionNextRound()
    {
        ...
        if (playerBidTurn == false)
        {
            if (!weaponScript.playerDictionary.ContainsKey(weaponPurchased))
            {
                ...
                if (!resultsScript.playerCollectionResults.ContainsKey(weaponPurchased) || !ResultsProcessor.activeWeaponNames.Contains(weaponDetails.itemName))
                {
                    resultsScript.playerCollectionResults.Add(weaponPurchased, weaponDetails);
                    ResultsProcessor.activeWeaponNames.Add(weaponDetails.itemName);
    ...
    public void TooBadLoser()
    {
        ...
        ResultsProcessor.activeWeaponNames.Remove(weaponDetails.itemName);
    ...
    private IEnumerator FleeceGetsTheWeapon()
    {
        ...
        if (fleeceFunds < 140 && roundTracker < numRounds)
        {
            FleeceTradeIn();
    ...
    public void TradeInWeapon(GameObject clickedWeapon)
    {
        ...
        if (weaponScript.playerDictionary.TryGetValue(clickedWeapon, out ItemInformation weaponDetails))
        {
            ...
            {
                moneyBack = (int)(weaponDetails.pricePaid * 0.8);
            }
    ...
    public void ConfirmTradeIn()
    {
        ...
        ResultsProcessor.activeWeaponNames.Remove(tradedWeaponDetails.itemName);
    ...
    private IEnumerator ProcessFleeceTrade()
    {
        ...
        resultsScript.fleeceScore -= weaponDetails.itemTier;
        Debug.Log("Fleece Score: " + resultsScript.fleeceScore);
        fleeceFunds += (int)(highestPricePaid * 0.8);

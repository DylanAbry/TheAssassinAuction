...
public class ResultsProcessor : MonoBehaviour
{
    ...
    public RoundManagement roundScript;
    ...
    private void Awake()
    {
        if (FindObjectsOfType<ResultsProcessor>().Length > 1)
        {
            Destroy(gameObject);
            return;
        }
        ...
        if (SceneManager.GetActiveScene().name == "WinEnding")
        {
            AssignToCanvas();
        }
    }
    ...
    {
        if (SceneManager.GetActiveScene().name == "AssassinAuctionMainGame")
        {
            Debug.Log("ForceReinitialize called");

            if (weaponScript == null)
            {
                weaponScript = FindObjectOfType<WeaponManager>();
                Debug.LogWarning("weaponScript was null! Auto-assigned: " + weaponScript);
            }

            if (roundScript == null)
            {
                roundScript = FindObjectOfType<RoundManagement>();
    ...
    private void AssignToCanvas()
    {
        ...
        if (canvas != null)
        {
            ...
            Debug.Log("ResultsProcessor re-parented to Canvas.");
        }
